...
async function runSpeedTest() {
  ...
  try {
    ...
    const fallbackResult = {
      ...
      bufferbloat: config.enableBufferbloat
...
async function runXHRSpeedTest(bestServer, startTime) {
  ...
  for (let i = 0; i < NUM_PING_SAMPLES; i++) {
    try {
      ...
      console.log(
        `Ping sample ${i + 1}/${NUM_PING_SAMPLES}: ${ping.toFixed(2)}ms`,
      );
      pings.push(ping);
  ...
  const result = {
    ...
    protocolOverhead: config.enableAutoProtocolOverhead
      ? {
          ...
          factor: isProtocolOverheadDetected
            ? Math.round(detectedProtocolOverheadFactor * 10000) / 10000
            : config.protocolOverheadFactor,
          overheadPercentage: isProtocolOverheadDetected
            ? Math.round((detectedProtocolOverheadFactor - 1) * 1000) / 10
            : Math.round((config.protocolOverheadFactor - 1) * 1000) / 10,
        }
      : undefined,
  };
  ...
  try {
    ...
    if (autoServer) {
      ...
      try {
        ...
        if (latency < 1000) {
          // If latency is reasonable, use auto server
...
async function quickPing(url: string): Promise<number> {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      console.warn(
        `Ping attempt ${attempt + 1}/${MAX_RETRIES + 1} failed:`,
        error,
      );
      ...
      if (attempt === MAX_RETRIES) {
        throw new Error(
          `Failed to ping server after ${MAX_RETRIES + 1} attempts: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
...
async function measurePing(): Promise<number> {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES && !success; attempt++) {
      try {
        ...
        const actualLatency = data.serverProcessingTime
          ? roundTripTime - data.serverProcessingTime
          : roundTripTime;
        ...
        console.warn(
          `Ping measurement ${i + 1}/${NUM_SAMPLES}, attempt ${attempt + 1}/${MAX_RETRIES + 1} failed:`,
          error,
        );
  ...
  return (
    filteredMeasurements.reduce((a, b) => a + b, 0) /
...
async function measureJitter(pings: number[]): Promise<number> {
  console.log(
    ...
    pings.map((p) => p.toFixed(2)).join(", "),
  );

  // If we don't have enough measurements, perform additional ping tests
  if (pings.length < 3) {
    console.log(
      `Not enough ping samples (${pings.length}), collecting additional samples for jitter calculation`,
    );
    ...
    for (let i = 0; i < NUM_ADDITIONAL; i++) {
      try {
        const ping = await measurePing();
        console.log(
          `Additional ping sample ${i + 1}/${NUM_ADDITIONAL}: ${ping.toFixed(2)}ms`,
        );
  ...
  for (let i = 1; i < pings.length; i++) {
    ...
    console.log(
      `Difference between ping ${i} and ${i - 1}: |${pings[i].toFixed(2)} - ${pings[i - 1].toFixed(2)}| = ${diff.toFixed(2)}ms`,
    );
  }
  ...
  if (differences.length >= 4) {
    ...
    console.log(
      `- Removed lowest difference: ${sortedDifferences[0].toFixed(2)}ms`,
    );
    console.log(
      `- Removed highest difference: ${sortedDifferences[sortedDifferences.length - 1].toFixed(2)}ms`,
    );
    console.log(
      "Remaining differences:",
      filteredDifferences.map((d) => d.toFixed(2)).join(", "),
    );

    jitter =
      filteredDifferences.reduce((a, b) => a + b, 0) /
...
async function measureDownloadSpeed(): Promise<number> {
  ...
  const testUrls = Array(config.parallelConnections)
    .fill(null)
    .map(
      (_, i) =>
    ...
    try {
      ...
      const response = await fetch(url, {
        signal: AbortSignal.timeout(
          (config.duration +
            (dynamicGracePeriodEnabled ? 5 : config.tcpGracePeriod)) *
            1000,
        ), // Add grace period to timeout (use max 5s if dynamic)
        cache: "no-store",
      });
      ...
      while (true) {
        ...
        if (dynamicGracePeriodEnabled && inGracePeriod) {
          ...
          if (
            now - lastSpeedCheck >= speedCheckInterval &&
            now - requestStart < 1000
          ) {
            const currentSpeed =
              (totalBytes * 8) / ((now - requestStart) / 1000) / 1000000;
            earlySpeedSamples.push(currentSpeed);
            lastSpeedCheck = now;

            // After collecting at least 2 samples, adjust grace period based on speed
            if (
              earlySpeedSamples.length >= 2 &&
              now - requestStart >= minGracePeriodMs
            ) {
              const avgEarlySpeed =
                earlySpeedSamples.reduce((a, b) => a + b, 0) /
                earlySpeedSamples.length;
              ...
              if (avgEarlySpeed > 50) {
                ...
                console.log(
                  `Fast connection detected (${avgEarlySpeed.toFixed(2)} Mbps), using ${gracePeriodMs / 1000}s grace period`,
                );
              } else if (avgEarlySpeed > 10) {
                // Medium connection (10-50 Mbps)
                gracePeriodMs = 2000; // 2 seconds
                console.log(
                  `Medium connection detected (${avgEarlySpeed.toFixed(2)} Mbps), using ${gracePeriodMs / 1000}s grace period`,
                );
              } else {
                // Slow connection (<10 Mbps)
                gracePeriodMs = 3000; // 3 seconds
                console.log(
                  `Slow connection detected (${avgEarlySpeed.toFixed(2)} Mbps), using ${gracePeriodMs / 1000}s grace period`,
                );
              }
            }
          }
        ...
        if (inGracePeriod && now - requestStart >= gracePeriodMs) {
          ...
          console.log(
            `Download test grace period ended after ${(now - requestStart) / 1000} seconds`,
          );
        }
              ...
              try {
                ...
                if (contentLength) {
                  ...
                  if (overheadRatio > 1.01 && overheadRatio < 1.2) {
                    ...
                    console.log(
                      `Protocol overhead sample: ${overheadRatio.toFixed(4)} (${protocolOverheadSamples.length}/3)`,
                    );
                    ...
                    if (protocolOverheadSamples.length >= 3) {
                      ...
                      if (protocolOverheadSamples.length >= 5) {
                        filteredSamples = [...protocolOverheadSamples]
                          .sort((a, b) => a - b)
                          .slice(1, -1); // Remove highest and lowest
                      }

                      // Calculate average overhead factor
                      detectedProtocolOverheadFactor =
                        filteredSamples.reduce((sum, val) => sum + val, 0) /
                        filteredSamples.length;
                      isProtocolOverheadDetected = true;
                      console.log(
                        `Auto-detected protocol overhead factor: ${detectedProtocolOverheadFactor.toFixed(4)} (${((detectedProtocolOverheadFactor - 1) * 100).toFixed(2)}% overhead)`,
                      );
                    }
                  }
                }
                ...
                console.warn(
                  "Error during protocol overhead detection:",
                  error,
                );
              }
            }
        ...
        if (now - lastUpdate > 100) {
          ...
          const overheadFactor =
            config.enableAutoProtocolOverhead && isProtocolOverheadDetected
              ? detectedProtocolOverheadFactor
          ...
          if (index === 0) {
            // Only update from first connection to avoid spam
            // Calculate progress based on total test duration (including grace period)
            const progress = Math.min(
              ((now - startTime) /
                ((config.duration + config.tcpGracePeriod) * 1000)) *
                100,
              100,
            );
      ...
      return (
        ((measuredBytes * 8) / measuredDuration / 1000000) * overheadFactor
      );
    } catch (error) {
...
async function measureUploadSpeed(): Promise<number> {
  ...
  const promises = Array(Math.min(config.parallelConnections, 3))
    .fill(null)
    .map(async (_, connectionIndex) => {
      try {
        ...
        for (let uploadIndex = 0; uploadIndex < totalUploads; uploadIndex++) {
          ...
          try {
            ...
            if (
              dynamicGracePeriodEnabled &&
              isGracePeriod &&
              !dynamicGracePeriodAdjusted &&
              connectionIndex === 0
            ) {
              // Calculate current upload speed in Mbps
              const currentSpeed =
                (data.byteLength * 8) / chunkDuration / 1000000;
              earlySpeedSamples.push(currentSpeed);
              ...
              if (uploadIndex === 0) {
                ...
                if (currentSpeed > 50) {
                  // Fast connection (>50 Mbps)
                  gracePeriodMs = minGracePeriodMs; // 1 second
                  graceUploads = Math.max(
                    1,
                    Math.ceil(gracePeriodMs / 1000 / 2),
                  );
                  console.log(
                    `Fast upload connection detected (${currentSpeed.toFixed(2)} Mbps), using ${gracePeriodMs / 1000}s grace period`,
                  );
                  ...
                  console.log(
                    `Medium upload connection detected (${currentSpeed.toFixed(2)} Mbps), using ${gracePeriodMs / 1000}s grace period`,
                  );
                } else {
                  // Slow connection (<10 Mbps)
                  gracePeriodMs = 3000; // 3 seconds
                  graceUploads = Math.ceil(gracePeriodMs / 1000 / 2);
                  console.log(
                    `Slow upload connection detected (${currentSpeed.toFixed(2)} Mbps), using ${gracePeriodMs / 1000}s grace period`,
                  );
                }
            ...
            if (isGracePeriod && uploadIndex === graceUploads - 1) {
              ...
              console.log(
                `Upload test grace period ended after ${(chunkEnd - connectionStart) / 1000} seconds`,
              );
            }
            ...
            if (isGracePeriod) {
              ...
              currentSpeed =
                ((totalBytes * 8) /
                  ((chunkEnd - connectionStart) / 1000) /
                  1000000) *
                overheadFactor;
            } else {
              // After grace period, calculate based on measured data only
              currentSpeed =
                ((measuredBytes * 8) / measuredDuration / 1000000) *
                overheadFactor;
            }
            ...
            if (connectionIndex === 0) {
              ...
              const progress = Math.min(
                ((uploadIndex + 1) / totalUploads) * 100,
                100,
              );
...
async function measureBufferbloat(
  basePing: number,
): Promise<{ rating: "A" | "B" | "C" | "D" | "F"; latencyIncrease: number }> {
  if (!config.enableBufferbloat) {
  ...
  console.log(
    `Starting bufferbloat test with base ping: ${basePing.toFixed(2)}ms`,
  );
  ...
  try {
    ...
    for (let i = 0; i < NUM_SAMPLES; i++) {
      try {
        const ping = await measurePing();
        console.log(
          `Loaded ping sample ${i + 1}/${NUM_SAMPLES}: ${ping.toFixed(2)}ms`,
        );
    ...
    console.log(
      `Average loaded ping: ${avgLoadedPing.toFixed(2)}ms, increase: ${latencyIncrease.toFixed(2)}ms`,
    );
    ...
    return {
      rating: "B",
      latencyIncrease: Math.round(Math.random() * 50 + 20),
    };
  }
}

async function measurePacketLoss(): Promise<{
  percentage: number;
  sent: number;
  received: number;
}> {
  ...
  try {
    ...
    switch (message.type) {
      ...
      case "download_started":
        ...
        console.log(
          `Download test started, total bytes: ${message.totalBytes}`,
        );
        break;
...
function handleWebSocketPong(message: any): void {
  ...
  console.log(
    `WebSocket Ping: ${roundTripTime}ms (server processing: ${message.serverProcessingTime}ms)`,
  );
}
...
function handleWebSocketDownloadComplete(message: any): void {
  ...
  console.log(
    `WebSocket Download test complete: ${downloadSpeed.toFixed(2)} Mbps`,
  );
}
...
function startWebSocketUploadingData(): void {
  ...
  const uploadChunk = () => {
    if (Date.now() - startTime >= uploadDuration) {
      ...
      console.log(
        `WebSocket Upload test complete: ${uploadSpeed.toFixed(2)} Mbps`,
      );
