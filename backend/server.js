...
function updateRequestStats(endpoint) {
  ...
  serverStats.lastMinuteRequests = serverStats.lastMinuteRequests.filter(
...
if (cluster.isPrimary && ENABLE_CLUSTER) {
  ...
  setInterval(() => {
    ...
    console.log(
      `[Master] Uptime: ${uptime.toFixed(0)}s | Workers: ${activeWorkers}/${numCPUs} | ` +
  ...
  console.log(
    `Performance settings: keepAliveTimeout=${keepAliveTimeout}ms, headersTimeout=${headersTimeout}ms`,
  );
  ...
  console.log(
    `Rate limiting: ${rateLimitConfig.max} requests per ${rateLimitConfig.windowMs / 1000}s`,
  );
  ...
  if (process.env.NODE_ENV === "production") {
    ...
    app.use(
      morgan("combined", {
        ...
        skip: (req) =>
          req.url.includes("/download") || req.url.includes("/upload"),
      }),
    );
  ...
  setInterval(() => {
    ...
    console.log(
      `[Worker ${cluster.worker.id}] Requests: ${serverStats.requestCount} | ` +
        `Req/s: ${requestsPerSecond.toFixed(2)} | ` +
  ...
  app.get("/ping", (req, res) => {
    ...
    const requestTimestamp = req.query.timestamp
    ...
    const responseData = {
      ...
      serverProcessingTime: requestTimestamp
        ? serverTimestamp - requestTimestamp
        : null,
    };
    ...
    if (includeLoad) {
      responseData.serverLoad = {
        ...
        requestsPerSecond: (serverStats.lastMinuteRequests.length / 60).toFixed(
          2,
        ),
      };
    }
  ...
  app.get("/download", (req, res) => {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="speedtest.bin"',
    );
    ...
    req.on("close", () => {
      if (bytesWritten < safeSize) {
        ...
        console.log(
          `Download aborted: ${(bytesWritten / (1024 * 1024)).toFixed(2)}MB of ${(safeSize / (1024 * 1024)).toFixed(2)}MB in ${duration.toFixed(2)}s (${throughput.toFixed(2)}MB/s)`,
        );
      }
    });
    ...
    function writeNextChunk() {
      if (bytesWritten >= safeSize) {
        ...
        console.log(
          `Download complete: ${(safeSize / (1024 * 1024)).toFixed(2)}MB in ${duration.toFixed(2)}s (${throughput.toFixed(2)}MB/s)`,
        );
        return res.end();
      }
  ...
  app.post("/upload", (req, res) => {
    ...
    if (req.headers["content-type"] === "application/octet-stream") {
      ...
      req.on("end", () => {
        ...
        console.log(
          `Upload complete: ${(totalBytes / (1024 * 1024)).toFixed(2)}MB in ${duration.toFixed(2)}s (${throughputMBps.toFixed(2)}MB/s)`,
        );
      ...
      req.on("close", () => {
        if (!res.headersSent) {
          ...
          const duration = endTime[0] + endTime[1] / 1e9;
          console.log(
            `Upload aborted: ${(totalBytes / (1024 * 1024)).toFixed(2)}MB in ${duration.toFixed(2)}s`,
          );
        }
      });
    } else {
  ...
  server.listen(PORT, () => {
    console.log(
      `Speed test backend server running on port ${PORT} (Worker ${cluster.worker.id})`,
    );
